...
def decode_gpt(tokenizer, model, input_ids, top_k, top_p):
    ...
    resulting_string = tokenizer.decode(output[0], skip_special_tokens=True)
    return resulting_string
...
def get_all_predictions(text_sentence, model_name, top_clean=5):
    ...
    #     return {'gpt': gpt}
        ...
        with torch.no_grad():
            prompt_length = len(
                xlnet_tokenizer.decode(input_ids[0], skip_special_tokens=True, clean_up_tokenization_spaces=True))
